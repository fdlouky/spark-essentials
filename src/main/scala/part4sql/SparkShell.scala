...
object SparkShell extends App {
  ...
  /**
    * This file contains a dummy application where I added what I learned in the Spark Shell lecture.
    */
  ...
  /**
    ...
    *   1. In another terminal window/tab, connecto to the Docker container and open a regular shell
    *     docker exec -it docker-spark-cluster_spark-master_1 bash
    *   2. Execute the Spark SQl shell
    *     /spark/bin/spark-sql
    *
    * If you want to inspect the files that Spark SQL writes, open another terminal windows/tab
    *     docker exec -it docker-spark-cluster_spark-master_1 bash
    *
    * The Spark SQL shell allows you to write any SQL statement, from creating databases and tables, to selecting,
    * to creating views, etc.
    *
    * As discussed in the lecture, there is no difference between Spark SQL tables and DataFrames from Spark's point
    * of view.
  ...
  /**
    ...
    *   - An EXTERNAL table means that Spark is only responsible for metadata, whereas data is stored somewhere else
    *     (i.e files, HDFS, other databases). When you drop an external table, Spark will only delete the metadata,
    *     meaning that you won't be able to reference or use it.
    ...
    *                                                          ^^ using CSV means store it in CSV from,
    ...
    * insert into persons_externak (select * from persons);
    */
